#include "ATA/config/cpp11_backward_compat.h"
#include "ATA/core/string.h"
#include <algorithm>
#include <cctype>
...
{
    const auto firstIt = std::find_if(m_string.begin(), m_string.end(), [](char32_t c)
    {
        return (c >= 256) || !std::isspace(static_cast<unsigned char>(c));
    });
    if (firstIt == m_string.end())
        return {};

    const auto lastIt = std::find_if(m_string.rbegin(), m_string.rend(), [](char32_t c)
    {
        return (c >= 256) || !std::isspace(static_cast<unsigned char>(c));
    }).base();
...
String::String(const std::string& str) :
    m_string(utf::UTF8_to_UTF32(str.begin(), str.end()))
{
}

String::String(const std::wstring& str) :
    m_string(utf::Wide_to_UTF32(str.begin(), str.end()))
{
}

String::String(const std::u16string& str) :
    m_string(utf::UTF16_to_UTF32(str.begin(), str.end()))
{
}
...
String::String(const char* str) :
    m_string(utf::UTF8_to_UTF32(str, str + std::char_traits<char>::length(str)))
{
}

String::String(const wchar_t* str) :
    m_string(utf::Wide_to_UTF32(str, str + std::char_traits<wchar_t>::length(str)))
{
}

String::String(const char16_t* str) :
    m_string(utf::UTF16_to_UTF32(str, str + std::char_traits<char16_t>::length(str)))
{
}
...
{
    return utf::UTF32_to_StdStringUTF8(m_string);
}

std::wstring String::toWideString() const
{
    return utf::UTF32_to_Wide(m_string);
}

std::u16string String::toUtf16() const
{
    return utf::UTF32_to_UTF16(m_string);
}
